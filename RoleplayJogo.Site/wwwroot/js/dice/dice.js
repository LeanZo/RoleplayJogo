...
(function(dice) {
    ...
    this.selector_back_color = 0xf8f0e9;
    this.desk_color = 0xf8f0e9;
    ...
    this.dice_box = function(container, dimentions) {
        ...
        this.renderer.setClearColor(0xf8f0e9, 1);
    ...
    this.dice_box.prototype.getDiceValues = function (dices) { return get_dice_values(dices) } 
    ...
    function shift_dice_faces(dice, value, res) {
        ...
        if (dice.dice_type == 'd100' && res == 100) res = 0;

        if (dice.dice_type == 'd100') {
            var valueString = value.toString().replaceAll('0', '');
            var resString = res.toString().replaceAll('0', '');

            if (valueString == '') valueString = '0';
            if (resString == '') resString = '0';

            value = parseInt(valueString);
            res = parseInt(resString);
        }
        if (!(value >= r[0] && value <= r[1])) return;
        var num = value - res;
        var geom = dice.geometry.clone();
        var matIndexOriginal = res + 1;
        var matIndexNovo = value + 1;
        for (var i = 0; i < geom.faces.length; i++) {
            ...
            if (matindex == matIndexOriginal) {
                geom.faces[i].materialIndex = matIndexNovo;
            } else if (matindex == matIndexNovo) {
                geom.faces[i].materialIndex = matIndexOriginal;
            }
        }
    ...
    this.dice_box.prototype.draw_selector = function() {
        ...
        this.pane = new THREE.Mesh(new THREE.PlaneGeometry(this.w * 6, this.h * 6, 1, 1), 
            new THREE.MeshBasicMaterial({ color: that.selector_back_color }));
